...
class RegisterScreenTests {
    ...
    @Ignore("Игнорировано из-за бага, так-как согласно документации пароль от 6 до 20, в app проходит 5")
    @Test
    fun errorMinLenPassword(){

        val email = Faker().internet().emailAddress()
        val password = Faker().internet().password(1, 5)
        val age = Faker().random().nextInt(0, 99).toString()
    ...
    @Ignore("Длина согласно документации <= 50 - не учтена - БАГ ")
    @Test
    fun errorMaxLenEmail() {

        val email = Faker().internet().emailAddress().take(51)
        val password = Faker().internet().password(6,20)
        val age = Faker().random().nextInt(0,99).toString()
